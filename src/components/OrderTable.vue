<!-- Custmers Order List page on web-->
<template>
        ...
        <a-table :columns="columns" :data-source="data">
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'shipping'">
              <span>
                <a-tag> Street Address :{{ record.streetAddress }} </a-tag>
                <a-tag> Town/City :{{ record.town }} </a-tag>
                <a-tag> Postcode :{{ record.postCode }} </a-tag>
              </span>
            </template>
            ...
            <template v-else-if="column.key === 'total_price'">
              <span> £{{ Math.round(record.total_price * 100) / 100 }} </span>
            </template>
            <template v-else-if="column.key === 'details'">
              <router-link
                :to="{
                  name: 'ShowDetailsOrder',
                  params: { nbr: record.numberOrder },
                }"
              >
